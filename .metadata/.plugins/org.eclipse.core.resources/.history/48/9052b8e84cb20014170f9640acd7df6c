...
{
	//os_printf("in pwm_set_duty\n");
	//os_printf("setting duty to: %d", duty);
	//os_printf("setting channel: %d", channel);
    if (duty < 1) {
        pwm.duty[channel] = 0;
...
{
    uint8 i;
    os_printf("in freq/duty. setting freq\r\n");
    pwm_set_freq(freq);

    for (i = 0; i < PWM_CHANNEL; i++) {
    	os_printf("setting duty %d of %d\n", i, PWM_CHANNEL);
        pwm_set_duty(0, i);
    }
}
...
{
    uint8 i;
    os_printf("inside init function\r\n");

    ETS_FRC_TIMER1_INTR_ATTACH(pwm_tim1_intr_handler, NULL);
    TM1_EDGE_INT_ENABLE();
    ETS_FRC1_INTR_ENABLE();

    os_printf("timer1 interupt enabled\r\n");
    ...
    os_printf("pin functions selected\r\n");

    for (i = 0; i < PWM_CHANNEL; i++) {
        pwm_gpio |= (1 << pwm_out_io_num[i]);
    }

    os_printf("about to set freq/duty\r\n");
    //pwm_set_freq_duty(freq, duty);
    pwm_set_freq_duty(500, (uint8 *)250);

    os_printf("pwm_set_freq_duty done\r\n");

    pwm_start();
    os_printf("pwm_start done\r");
}
